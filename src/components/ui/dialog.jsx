...
const DialogOverlay = React.forwardRef(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ref={ref}
    className={cn(
      "data-[state=closed]:animate-out data-[state=closed]:fade-out data-[state=open]:fade-in fixed inset-0 z-9999  bg-slate-900 bg-opacity-30  transition-all duration-100",
      className
    )}
    {...props}
  />
  ...
  ({ className, children, ...props }, ref) => (
    <DialogPortal>
      ...
      <DialogPrimitive.Content
        ...
        ref={ref}
        className={cn(
          "animate-in data-[state=open]:fade-in-90 data-[state=open]:slide-in-from-bottom-10 sm:zoom-in-90 data-[state=open]:sm:slide-in-from-bottom-0 fixed z-9999 grid w-full gap-4 rounded-b-lg border bg-white p-6 shadow-lg sm:max-w-lg sm:rounded-lg",
          className
        )}
        {...props}
      >
